...
export async function saveOtp(email, code) {
    ...
    if (isCloud) {
        try {
            // Check if user exists, if not we can't save OTP to user table easily unless we have a separate OTP table or create a temp user.
            // For this simple flow, we'll assume we upsert or create a placeholder.
            // Actually, for registration, the user doesn't exist yet.
            // So we need a separate 'otps' table or store it in a way that doesn't require a user ID.
            // Let's create an 'otps' table.
            ...
            await sql`
                ...
                VALUES (${email}, ${code}, ${expires.toISOString()})
                ON CONFLICT (email) DO UPDATE SET code = ${code}, expires = ${expires.toISOString()};
            `;
            return true;
        } catch (error) {
            console.error('Postgres Error saveOtp:', error);
            return false;
        }
    } else {
        const db = readLocalDb();
        if (!db.otps) db.otps = {};
        db.otps[email] = { code, expires: expires.toISOString() };
        writeLocalDb(db);
        return true;
    }
}
...
export async function verifyOtp(email, code) {
    if (isCloud) {
        try {
            const { rows } = await sql`SELECT * FROM otps WHERE email = ${email} AND code = ${code} AND expires > NOW();`;
            return rows.length > 0;
        } catch (error) {
            console.error('Postgres Error verifyOtp:', error);
            return false;
        }
    } else {
        const db = readLocalDb();
        if (!db.otps || !db.otps[email]) return false;
        const record = db.otps[email];
        const isValid = record.code === code && new Date(record.expires) > new Date();
        return isValid;
    }
}
